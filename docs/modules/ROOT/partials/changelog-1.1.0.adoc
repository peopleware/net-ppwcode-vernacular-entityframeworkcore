////
The changelog for a specific release.
Note that it is best to update the changelog as new code is developed.

All attributes used, must be included in this file to keep it self-contained.

This file is translated into markdown and used for the release notes in the GitHub release!
////

=== Changes
* Override SavingChanges for `WildExceptionsInterceptor` / `AuditableInterceptor` (see link:{github-issue}1[#1])
* Switched the solution file from `.sln` to `.slnx` format
* Updated the .NET SDK version to `10.0.302`
* Added `gen5` to the NuGet package tags
* Generated portable debug symbols for both Debug and Release configurations
* Changed test and analyzer package versions to floating versions
* Updated test project dependencies to the latest available versions
* Remove Microsoft.SourceLink.GitHub from global package references 
* Update Microsoft.Data.SqlClient package to version 7.*
